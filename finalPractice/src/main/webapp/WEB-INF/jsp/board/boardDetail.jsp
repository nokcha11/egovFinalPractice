...
<script type="text/javascript">
		    ...
		    success: function (data, status, xhr) {
		    	...
		    	if(data.replyList.length > 0){
		    		for(var i=0; i<data.replyList.length; i++){
			    		...
			    		replyHtml += '답글달기';
			    		replyHtml += '</a><br>';
			    		// boardIdx 값 주기 방법1 , \'' + boardIdx + '\' 추가해서 더하기
			    		replyHtml += '<a href="javascript:fn_replyDelete(\'' + data.replyList[i].replyIdx + '\', \'' + boardIdx + '\');" style="float:right;">';
			    		replyHtml += '삭제';
			    		replyHtml += '</a>';
			    		replyHtml += '</span></br>';
			    		replyHtml += '</div>';	
	...
	// boardIdx 값 주기 방법1 -> fn_replyDelete(replyIdx,boardIdx)
	function fn_replyDelete(replyIdx,boardIdx){
		//boardIdx 값 주기 방법2 var boardIdx = $("#boardIdx").val();
		...
		$.ajax({
		    ...
		    method: 'post',
		    data : { 
		    	//"boardIdx" : boardIdx, 값을 안 넣어줘도 무방
		    	"replyIdx" : replyIdx
		    },
		    dataType : 'json',
		    success: function (data, status, xhr) {
